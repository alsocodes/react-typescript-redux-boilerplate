...
const GridData2: FC<Props> = ({
  ...
  // useEffect(() => {
  //   if (data) setGdData([...data]);
  // }, [data]);

  // const textSize = 'text-sm leading-5';
  // const paddingHeader = pdHeader || '';
  // const paddingBody = pdBody || '';
  ...
  useEffect(() => {
    setTCols(
      cols.map((item: any, i: number) => {
        ...
        const left = cols
          .filter((f: any, x: number) => x < i, [])
          .reduce(
            (t: number, r: any) => t + (r.width !== undefined ? r.width : autoWidth),
            0,
          );
        return {
          ...item,
  ...
  // console.log(totalData / perPage, totalData, perPage);
    ...
    <div className="dg border border-gray-300" ref={gridDataRef}>
          ...
          {tCols?.map((item: any, i: number) => {
            ...
            return (
              <div
                ...
                key={label}
                className={`${'dg-header-col px-2 border-b border-gray-300'}`}>
                {label}
              </div>
            );
          })}
                ...
                {tCols?.map((col: any, x: number) => {
                  ...
                  return (
                    <div
                      style={{ width: width, left: left }}
                      key={`${label}-${x}`}
                      className={`dg-body-col px-2 border-b border-gray-300 ${''} `}>
                      {render ? render(item, i) : item[name]}
                    </div>
                  );
                })}
              </div>
      ...
      <div className="dg-paging py-1 border-t border-gray-300">
        <div className="flex gap-0 px-1">
          <button
            className={`btn btn-xs btn-ghost border text-gray-600 ${
          ...
          <div className="form-control">
            <input
              ...
              className="bg-white input-number border border-gray-400 p-1 text-xs"
            />
          </div>
          ...
          <div className="form-control">
            <select
              ...
              className="paging-select bg-white border border-gray-400 p-1 text-xs">
          ...
          <div className="text-gray-600 panging-text-info px-1">
            Showing {skip + 1} - {skip + perPage > totalData ? totalData : skip + perPage}{' '}
            of {totalData}
          </div>
        </div>
      </div>
